...
struct GameView: View {
    ...
    var body: some View {
        ZStack {
            CardView(card: Card(number: 0, flipped: true))
        ...
        }.background(Image("wood_texture").aspectRatio(contentMode: /*@START_MENU_TOKEN@*/.fill/*@END_MENU_TOKEN@*/).ignoresSafeArea())
        .onAppear {
            createPlayerViews() // Create PlayerView instances when the view appears
            createCenterCards()
        }
    }
    ...
    func createPlayerViews() {
        ...
        for playerIndex in 0..<game.players.count {
            ...
            yOffset += 100
            if playerIndex == 0 {
                player.position = CGPoint(x: UIScreen.main.bounds.width / 2, y: UIScreen.main.bounds.height - 60)
            }
